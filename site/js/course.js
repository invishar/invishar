/* =============================================================================
   ...
   Sumber isinya dua lapis:
     1. data/course-<slug>.json — terbitan panel admin, kalau ada
     2. js/course-data.js       — data bawaan, cadangan kalau (1) belum ada
   Dengan begitu situs tetap hidup walau panel belum dipasang atau sedang mati.

   Kelas mana yang dibuka ditentukan oleh ?k=<slug> di alamat halaman.
   Catatan progres disimpan di localStorage peramban pengunjung.
   ============================================================================= */
(function () {
  "use strict";
  ...
  function ambilJson(alamat) {
    return fetch(alamat, { cache: "no-cache" }).then(function (jawab) {
  ...
  function muatKelas() {
    if (slug) {
      return ambilJson("data/course-" + encodeURIComponent(slug) + ".json")
        .catch(function () { return window.INVISHAR_COURSE; });
    }

    // Tanpa ?k=, kelas pertama pada daftar terbitan yang dibuka.
    return ambilJson("data/kelas.json")
      .then(function (daftar) {
        var pertama = (daftar.daftar || [])[0];
        if (!pertama) throw new Error("daftar kosong");
        slug = pertama.slug;
        return ambilJson("data/course-" + encodeURIComponent(slug) + ".json");
      })
      .catch(function () {
        slug = "";
        return window.INVISHAR_COURSE;
      });
  }

  muatKelas().then(function (kelas) {
    if (kelas) jalankan(kelas);
  });
  ...
  function jalankan(kelas) {
    ...
    /* --------------------------------------------------- Catatan progres */
    /* Kunci mengandung slug, jadi progres tiap kelas terpisah. Saat panel
       sudah punya akun pengguna, dua fungsi di bawah ini yang diganti
       pemanggilan ke API — sisanya tidak perlu disentuh. */
    ...
    var imbuhan = slug ? "&k=" + encodeURIComponent(slug) : "";

    function tautanMateri(id) {
      return "materi.html?m=" + encodeURIComponent(id) + imbuhan;
    }

    function tautanKelas(tambahan) {
      return "course.html" + (slug ? "?k=" + encodeURIComponent(slug) : "") + (tambahan || "");
    }

    /* ===================================================================
       Halaman kelas — course.html
       =================================================================== */
    function halamanKelas() {
      ...
      var ikhtisar = kelas.ikhtisar || {};
      isiDaftar($("#c-hasil"), ikhtisar.hasil);
      isiDaftar($("#c-untuk"), ikhtisar.untukSiapa);
      isiDaftar($("#c-syarat"), ikhtisar.syarat);
      ...
      function segarkanProgres() {
        ...
        var tombol = $("#c-lanjut");
        tombol.href = berikut ? tautanMateri(berikut.data.id) : "#materi";
      ...
      // Progres bisa berubah di tab lain (halaman materi).
      ...
      function pilihTab(tab, geser) {
        ...
        if (history.replaceState) history.replaceState(null, "", location.search + "#" + nama);
        if (geser) {
    ...
    function halamanMateri() {
      ...
      var tombolPutar = document.createElement("button");
      tombolPutar.className = "player-sampul";
      tombolPutar.type = "button";
      tombolPutar.setAttribute("aria-label", "Putar video: " + mat.judul);

      var gambar = document.createElement("img");
      gambar.src = "https://i.ytimg.com/vi/" + mat.youtube + "/hqdefault.jpg";
      gambar.alt = "";
      gambar.loading = "lazy";
      tombolPutar.appendChild(gambar);

      var lencana = buat("span", "player-tombol");
      lencana.setAttribute("aria-hidden", "true");
      tombolPutar.appendChild(lencana);
      ...
      wadahPlayer.appendChild(tombolPutar);
      ...
      function segarkan() {
        ...
        $(".paham").classList.toggle("is-on", paham);
      ...
      var kotakDaftar = $(".daftar-isi");
      if (aktif && kotakDaftar.scrollHeight > kotakDaftar.clientHeight) {
        kotakDaftar.scrollTop = aktif.offsetTop - 80;
      }
